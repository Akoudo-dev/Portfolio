...
export default function Projects() {
    ...
    <h2 className="text-xl sm:text-2xl md:text-3xl font-medium text-center">Projets qui ont <span className="text-[#0f4cf7]">cartonné</span></h2>
    ...
    <article key={p.id} className="group p-4 sm:p-6 hover:shadow-lg transition-all hover:scale-105 duration-300">
      <div className="relative overflow-hidden rounded-xl sm:rounded-2xl">
        <img src={p.image} alt={`Aperçu du projet ${p.title}`} className="w-full h-auto border text-[#0f4cf7] bg-black p-3 sm:p-4 group-hover:brightness-75 transition-all" />
        ...
        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
          <a 
            ...
            className="bg-[#0f4cf7] text-black px-4 sm:px-6 py-2 sm:py-3 rounded-lg font-medium flex items-center gap-2 hover:bg-[#0f4cf7] transition-colors"
          >
            Voir le site
      ...
      <a 
        ...
        target="_blank" 
        rel="noopener noreferrer"
        className="inline-flex items-center gap-2 mt-3 sm:mt-4 text-[#0f4cf7] text-xs sm:text-sm hover:underline"
      >
        Visiter le projet
